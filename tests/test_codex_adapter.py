...
def test_read_env_no_profiles(codex_adapter):
    result = codex_adapter.read_env({"model_providers": {"databricks": {}}})
    assert result == {}


def test_read_env_no_auth_block(codex_adapter):
    data = {"profiles": {"default": {"model": "my-model", "model_provider": "databricks"}}}
...
def test_discover_config_finds_toml(tmp_path):
    """discover_config should find FMAPI config in a TOML file."""
    adapter = CodexAdapter()

    # Create config.toml with FMAPI provider
    config_dir = tmp_path / ".codex"
    config_dir.mkdir()
    config_file = config_dir / "config.toml"

    toml_data = {
        "profiles": {"default": {"model_provider": "databricks", "model": "databricks-gpt-5-2"}},
        "model_providers": {
            "databricks": {
                "name": "Databricks FMAPI",
                "base_url": "https://example.com/serving-endpoints/openai/v1",
    ...
    # Create a helper script with FMAPI_HOST and FMAPI_PROFILE
    helper_path = tmp_path / ".codex" / "fmapi-key-helper.sh"
    helper_path.write_text(
        '#!/bin/sh\nFMAPI_HOST="https://example.com"\nFMAPI_PROFILE="test-profile"\n'
    )

    # Patch settings_candidates to return our test path
...
def test_discover_config_toml_gateway(tmp_path):
    ...
    toml_data = {
        "profiles": {"default": {"model_provider": "databricks", "model": "test-model"}},
        "model_providers": {
            "databricks": {
